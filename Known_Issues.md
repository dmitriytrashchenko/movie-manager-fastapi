...
## Нет UI — только backend API
Проект сейчас чисто API-бэкенд, взаимодействие только через Swagger UI (`/docs`) или curl/Postman — нет ни одной пользовательской страницы. Пользователь ожидал увидеть готовое приложение, был удивлён "сырым" видом. Идея на будущее: добавить простой веб-фронтенд (список фильмов, форма добавления/редактирования, логин) поверх существующего API, чтобы это выглядело и ощущалось как настоящее приложение, а не только API. Не начато.
